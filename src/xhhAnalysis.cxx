...
bool m_vetoResolved = false;
bool m_doResolvedOnly = false;
bool m_doMergedOnly = false;
bool m_doFullyMergedOnly = false;
bool m_doCombined = false;
bool m_useAllR04 = true;
std::string m_calib = "";
bool m_corrMuInJet = true;
bool m_debug = false;
bool m_test = false;
...
{ 
	...
	if(!m_doResolvedOnly && !m_doMergedOnly && !m_doCombined)
	{
		std::cout<<"You didn't tell me what to do! Append --resolved, --merged or --combined to command."<< std::endl;
		return 1;
	}
	if((m_doResolvedOnly && m_doMergedOnly) || (m_doResolvedOnly && m_doCombined) || (m_doMergedOnly && m_doCombined))
	{
		std::cout<<"Multiple analysis types selected! Please choose only one!"<< std::endl;
		return 2;
	}
...
void analyseEvents(std::vector<std::pair<std::string, std::pair<double, double> > >::const_iterator fileIt)
{
	...
	for(auto syst: systList)
	{
		...
		HiggsCandidateProducer resolvedProducer(event);
		HiggsCandidateProducer mergedProducer(event);
		...
		for(unsigned long i = 0; i < maxEvents; ++i)
		{
			...
			std::vector<HiggsCandidate> resolvedHiggs;
			if(!m_doMergedOnly) resolvedHiggs = resolvedProducer.produceResolved(ak4Jets, muons, 0., 14000.);
			std::vector<HiggsCandidate> mergedHiggs;
			if(!m_doResolvedOnly)
			{
				if(m_useAllR04)
				{
					float mergedPtMin = 400;
					mergedHiggs = mergedProducer.produceMerged(ak4Jets, muons, mergedPtMin);
				}else
				{
					float mergedPtMin = 250;
					mergedHiggs = mergedProducer.produceMerged(caloR10, muons, mergedPtMin);
				}
			}
			if(m_debug) std::cout<<"analyseEvents: Produced "<< resolvedHiggs.size() <<" resolved candidates and "
									<< mergedHiggs.size() <<" merged."<< std::endl;
			std::array<HiggsCandidate, 2> combinedHiggs;
			if(m_doResolvedOnly)
			{
				if(resolvedHiggs.size() < 2) continue;  
				combinedHiggs = {{resolvedHiggs[0], resolvedHiggs[1]}};
			}else if(m_doMergedOnly)
			{
				if(mergedHiggs.size() < 2) continue;
				combinedHiggs = {{mergedHiggs[0], mergedHiggs[1]}};
				if(combinedHiggs[0].Pt() < 450.) continue;
			}else if(m_doCombined)
			{
				if(resolvedHiggs.size() + mergedHiggs.size() < 2) continue;
				std::vector<HiggsCandidate> combinedHiggsVector = chooseHiggsCandidates(resolvedHiggs, mergedHiggs);
				if(combinedHiggsVector.size()==2)
				{
					combinedHiggs = {{combinedHiggsVector[0], combinedHiggsVector[1]}};
				}else{
					continue;
				}
			}
			...
			if(m_doResolvedOnly) tops = topProducer.produce(combinedHiggs, ak4Jets); 
			if(event.isMC())
			{
				if(isSignal(fileName))
				{
					...
					for(auto& higgs: combinedHiggs)
					{
						...
						}else
						{
							if(m_useAllR04)	
							{
								for(int idx: higgs.getFatJet()->partonIdx())
								{
									indices.emplace_back(idx);
								}
							}else{//R=1.0 jets do not have the indices of matched particles stored.
								std::vector<Particle> bQuarks = partProducer.produce(5);		
								for(auto b : bQuarks)
								{
									if(b.deltaR(higgs) < higgs.getFatJet()->rParam())
									{
										indices.emplace_back(b.index());
									}
								}
							}
						}
					...
					//std::cout<<"analyseEvents: produceByIndex has created "<< jetPartons.size() <<" jet parton particles" << std::endl;
					std::vector<const Particle*> hDaughters = matchBToH(truthHiggs, jetPartons);
					//std::vector<const Particle*> hDaughters = matchBToH(truthHiggs, bQuarks);
...
std::vector<HiggsCandidate> chooseHiggsCandidates(const std::vector<HiggsCandidate>& resolvedHiggs,
													 const std::vector<HiggsCandidate>& mergedHiggs)
{
	std::vector<int> skipInMerged;
	std::vector<HiggsCandidate> toCombine;
	for(auto resolved: resolvedHiggs)
	{
		for(unsigned int m = 0; m < mergedHiggs.size(); ++m)
		{
			if(mergedHiggs[m].deltaR(resolved) < 0.35)
			{
				//First attempt, simply use resolved when there is any overlap.
				skipInMerged.emplace_back(m);
			}
		}	
		toCombine.emplace_back(resolved);
	}
	for(unsigned int m = 0; m < mergedHiggs.size(); ++m)
	{
		if(std::find(skipInMerged.begin(), skipInMerged.end(), m) != skipInMerged.end()) continue;
		toCombine.emplace_back(mergedHiggs[m]);
	}
    std::function<float(const HiggsCandidate&, const HiggsCandidate&)> bestPairChooser;
    bestPairChooser = &calcDhhScore;
	std::vector<HiggsCandidate> bestPair = chooseBestPair(toCombine, bestPairChooser);
	std::sort(bestPair.begin(), bestPair.end(), [](const HiggsCandidate a, const HiggsCandidate b)
    {
        return b.Pt() < a.Pt();
	});
	return bestPair;
...
int processCommandLine(int argc, char** argv)
{
	...
	{
		if(std::string(argv[i]) == std::string("--merged"))
		{
			m_doMergedOnly 	= true;
			m_doCombined 	= false;
			m_outputFileName = "EventAnaResults_merged";
		}else if(std::string(argv[i]) == std::string("--resolved"))
		{
			m_doResolvedOnly 	= true;
			m_doCombined 	= false;
			m_outputFileName = "EventAnaResults_resolved";
		}else if(std::string(argv[i]) == std::string("--combined")) 
		{
			m_doCombined 	= true;
			m_outputFileName = "EventAnaResults_combined";
		}
		else if(std::string(argv[i]) == std::string("--noSyst")) m_doSystematics = false;
		else if(std::string(argv[i]) == std::string("--R10")) m_useAllR04 = false;
		...
		else if(std::string(argv[i]) == std::string("--noMuInJet")) m_corrMuInJet = false;
		...
		else if(std::string(argv[i]) == std::string("--vetoResolved")) m_vetoResolved = true;
		else
		{
	...
	if(!m_corrMuInJet) m_outputFileName += "_noMuCorr";
	if(!m_useAllR04)	m_outputFileName += "_R10";
...
void setTruthHiggsId(HiggsCandidate& hcand, const std::vector<const Particle*>& hDaughters)
{
	...
	if(hcand.category() == 0) //Check that each jet can be matched to a different daughter from the same Higgs
	{
	...
	}else //Check for two daughters from the same Higgs boson matched to the same candidate.
	{
		...
		for(unsigned int b = 0; b < hDaughters.size(); ++b)
		{
			...
			if(hcand.deltaR(*(hDaughters[b])) < hcand.getFatJet()->rParam())
			{
				matches.push_back(b);
			}
		}
		if(matches.size() < 2) //Jet does not have two matched Higgs daughters, so set HiggsParentId to -98.
		{
			hcand.setTruthHiggsId(-98);
			return;
		}
		...
		//If we reach here, jet contains Higgs daughters, but they are not from the same Higgs
		hcand.setTruthHiggsId(-97);
	}	
}
